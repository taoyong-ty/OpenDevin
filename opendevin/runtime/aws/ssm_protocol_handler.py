...
class SSMProtocolHandler:
    ...
    def __init__(self, socket, session_id, token_value, stream_url):
        logger.info('Starting SSMProtocolHandler')
        self.socket = socket
    ...
    def _generate_init_message(self, client_width, client_height):
        return serialize_message(
            build_agent_message(
                payload=json.dumps({'cols': client_width, 'rows': client_height}),
    ...
    def _generate_input_message(self, text):
        return serialize_message(
            build_agent_message(
                payload=text,
                message_type='input_stream_data',
    ...
    @staticmethod
    def _generate_ack(message_type, sequence_number, message_id):
        return serialize_message(
            build_agent_message(
                payload=json.dumps(
                    {
    ...
    # Send text over self.sockets
    def send_text(self, text):
        self.sequence_number += 1
        return self.send(self._generate_input_message(text))

    def deserialize_message(self, input):
        logger.debug('deserialize: %s' % input)
